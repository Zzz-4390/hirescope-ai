...
export class CodeReviewProcessor {
  constructor(
    ...
    private readonly reviewer: CodeReviewGenerator,
  ...
  async process(taskId: string): Promise<void> {
    const task = await this.prisma.asyncTask.findUnique({ where: { id: taskId }, include: { codeReview: { select: { id: true, status: true } }, project: { select: { id: true, status: true, extractStoragePath: true, analysis: { select: { summary: true, techStack: true, directoryTree: true, coreModules: true, entryFiles: true, statistics: true } } } } } });
