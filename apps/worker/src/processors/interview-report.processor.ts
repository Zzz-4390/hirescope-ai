...
export class InterviewReportProcessor {
  ...
  async process(taskId: string): Promise<void> {
    ...
    if (questions.length !== task.interview.questionCount || answers.length !== task.interview.questionCount) return this.fail(task.id, task.interviewId, task.projectId, 'INTERVIEW_REPORT_INPUT_INVALID');
    ...
    try {
      ...
      return this.fail(task.id, task.interviewId, task.projectId, 'INTERVIEW_REPORT_GENERATION_FAILED');
    }
    const parsed = InterviewReportResultSchema.safeParse(candidate);
    if (!parsed.success || parsed.data.questionReviews.length !== task.interview.questionCount || parsed.data.questionReviews.some((review) => !review.rubric || !review.answerEvidence)) return this.fail(task.id, task.interviewId, task.projectId, 'INTERVIEW_REPORT_RESULT_INVALID');
  ...
  private claim(taskId: string, interviewId: string, projectId: string): Promise<boolean> {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (deleting(locked.projectStatus)) { await cancel(tx, taskId, interviewId); return false; }
  ...
  private finish(taskId: string, interviewId: string, projectId: string, userId: string, report: InterviewReportResult): Promise<void> {
    return this.prisma.$transaction(async (tx) => {
      const locked = await lockRows(tx, taskId, interviewId, projectId);
      if (!locked) throw new Error('TASK_NOT_FOUND');
      if (deleting(locked.projectStatus)) return cancel(tx, taskId, interviewId);
...
async function cancel(tx: Prisma.TransactionClient, taskId: string, interviewId: string): Promise<void> {
  await tx.interview.updateMany({
    where: { id: interviewId, status: InterviewStatus.REPORT_GENERATING },
