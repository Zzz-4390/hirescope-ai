...
export class TaskRecoveryService {
  ...
  private async markFailed(tx: Prisma.TransactionClient, task: RecoverableTask, code: string): Promise<void> {
    ...
    if (task.type === TaskType.PROJECT_CLEANUP && task.projectId) {
      await tx.project.updateMany({ where: { id: task.projectId, status: ProjectStatus.DELETING }, data: { status: ProjectStatus.FAILED, failureCode: code, failureMessage: '项目清理任务恢复失败' } });
    }
    if (task.type === TaskType.CODE_REVIEW && task.codeReviewId) {
