...
function FailurePanel({ message, onRetry, isRetrying = false }: { message: string; onRetry?: () => void; isRetrying?: boolean }) {
  return <div className="empty-panel interview-report-error-panel"><AlertCircle aria-hidden="true" /><h2>报告暂不可用</h2><p>{message}</p>{onRetry ? <button className="primary-button compact" type="button" disabled={isRetrying} onClick={onRetry}>{isRetrying ? <Loader2 aria-hidden="true" /> : <RefreshCw aria-hidden="true" />}重新生成报告</button> : null}</div>;
}
